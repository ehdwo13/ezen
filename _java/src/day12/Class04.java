...
public class Class04 {
	 ...
	 * 	- 객체없이 클래스만으로 사용가능.
	 * 	- 클래스명.메서드 / 클래스명.멤버변수
	 * 	- 객체를 생성해서 호출도 가능하지만 그렇게 사용하지 않음. (노란밑줄:의미없음)
	 *	- 클래스의 멤버변수/메서드는 하나의 멤버변수가 모든 객체에 공유(사용)할때 사용
	 * 
	 * 	static이 안붙은 메서드/멤버변수는 객체(인스턴스)의 멤버변수/메서드라고 한다.
	 * 	- 객체의 멤버변수/메서드는 객체를 통해 사용되고, 생성이 된다.(객체를 만들지 않으면 생성이 안됨)
	 * 	- 객체명.메서드 / 객체명.멤버변수
	 *  - 각 객체마다 독립적인 객체 => 각 객체마다 독립적인 멤버변수, 메서드를 생성
	 *  
	 * 	- 클래스 / 객체는 생성 시점이 달라서 메서드를 사용할수 있는 환경이 다르다.
	 * 	- 객체의 멤버는 클래스가 생성되고난 후 객체를 생성할수 있음.
	...
	public static void main(String[] args) {
		//	클래스 멤버에 접근하는 방식
		//현시점에서 BRAND는 이미 생성되어 있음. (객체가 없어도 생성)
...
//		t.printBrand();		//객체를 통해 클래스멤버에 접근할 수 있지만, 그렇게 사용하지않음
	}
